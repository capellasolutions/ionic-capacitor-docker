...
# The default stages the Android 37 toolchain so the image already has it the day Capacitor can target it. It cannot
# yet: Capacitor 8 compiles against a plain integer compileSdkVersion/targetSdkVersion of 36, and Google never
# published a plain "platforms;android-37" — only the minor releases 37.0 / 37.1 / 37.2, which AGP reaches through a
# separate compileSdkMinor that Capacitor does not expose. So 36 stays the pinned target and 37 rides along unused.
# When Capacitor does support it, move 37.2 / 37.0.0 up into the two ARGs above. Set this to "" to install nothing
# extra, or reuse it for any other package ("ndk;...", "cmake;...", "emulator", ...).
