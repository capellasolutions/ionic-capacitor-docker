...
# Resolve how to run the build script and the Capacitor CLIs for the selected package manager. The Capacitor CLI
# (@capacitor/cli) and @capacitor/assets are project-local devDependencies, so they run through the package manager's
# executor, not a global install. Note: pnpm and yarn forward extra args straight to the script, so a literal `--` would
...
# Start from a clean slate so `cap add` regenerates the native project deterministically (appId in capacitor.config.ts
# — already injected in prepare-build — becomes the Android namespace/applicationId and the iOS bundle id).
...
# Icons & splash from assets/. Non-fatal: a missing/odd asset must not fail the build — Capacitor falls back to its
# default launcher icons.
$assets generate --"$platform" || echo "Asset generation skipped for $platform."

if [ "$platform" = "android" ]; then
  # Firebase config: inert unless the Google Services Gradle plugin is wired in, but copied into the native project for
  # parity with the Cordova template.
  [ -f google-services.json ] && cp google-services.json android/app/google-services.json || true

  $cap sync android

  # Sign the release with Gradle's injected signing properties so the generated android/ project needs no hand-edits to
  # build.gradle. Values come from keystore.properties.
