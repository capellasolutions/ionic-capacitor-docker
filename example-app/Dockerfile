...
# Add package.json, lockfiles and the package-manager config first (before the rest of the
# project) so the dependency-install layer is cached and only re-runs when dependencies
# change. pnpm-workspace.yaml / .npmrc must be here too: pnpm 11 reads allowBuilds and
# nodeLinker (hoisted) from them, and without those the install leaves build scripts ignored
# (non-zero exit) and an un-hoisted node_modules that breaks Capacitor plugin discovery.
# The trailing * makes each optional, so npm/yarn projects without them still build.
...
# (This demo defaults to pnpm; an .npmrc with node-linker=hoisted makes pnpm's node_modules
# flat enough for Capacitor to discover plugin android/ ios/ folders during `cap sync`.)
RUN \
...
# Capacitor replaces Cordova's config.xml: inject the bundle id into capacitor.config.ts.
# This MUST happen before `cap add` (in the build stages) because appId becomes the Android
# namespace/applicationId and the iOS bundle id when the native project is generated.
...
# Build the Angular web app, generate the native Android project with Capacitor, copy the
# web assets + plugins in (cap sync), then build a signed release with Gradle. See
# scripts/build-capacitor.sh for the full flow. Run via bash so a missing +x bit on Windows
# checkouts doesn't matter.
...
# iOS cannot be compiled on Linux; we only prepare the Xcode project (cap add + cap sync,
# which runs pod install) for a macOS runner to finish.
